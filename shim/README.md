...
| `../scripts/create-vrt-clocks.sh` | Creates one netdevsim mock PHC per Kind worker and writes the device path into each node |
...
4. linuxptp image must contain the shim + wrapper (see ptp-operator `Dockerfile.lptpd`).
5. Tests: set `DisableAllSlaveRTUpdate: false` (netdevsim CI default when `PTP_VRT_ENABLE=true`).
