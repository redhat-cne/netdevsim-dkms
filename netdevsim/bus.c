...
nsim_bus_dev_new(unsigned int id, char *bus, int clk_id, unsigned int port_count,
		 unsigned int num_queues, unsigned int wpc);

static ssize_t
new_device_store(const struct bus_type *bus, const char *buf, size_t count)
{
	unsigned int id, port_count, num_queues, wpc;
	int clk_logical_id;
	char *bus_str=kzalloc(32, GFP_KERNEL);
	struct nsim_bus_dev *nsim_bus_dev;
	int err;

	pr_info("netdevsim: scanning pci address %s", buf);
	err = sscanf(buf, "%u %s %d %u %u %u", &id, bus_str, &clk_logical_id, &port_count, &num_queues, &wpc);
	switch (err) {
	case 1:
		strcpy(bus_str,"0000:00:01.0");
		fallthrough;
	case 2:
	    clk_logical_id = -1;
		fallthrough;
	...
	case 6:
		if (id > INT_MAX) {
			pr_err("netdevsim: Value of \"id\" is too big.\n");
			return -EINVAL;
		}
		break;
	default:
		pr_err("Format: \"id [pci_addr] [clk_id] [port_count] [num_queues] [wpc]\"\n");
		return -EINVAL;
	}
	...
	nsim_bus_dev = nsim_bus_dev_new(id, bus_str, clk_logical_id, port_count,
					num_queues, wpc);
...
/* Parse PCI address string (dddd:xx:yy:z format) */
static int parse_pci_address(const char *str, struct pci_addr *pci_addr)
{
	unsigned int domain, bus, device, function;

	if (sscanf(str, "%04x:%02x:%02x.%01x", &domain, &bus, &device, &function) != 4) {
		pr_err("netdevsim: Failed to parse PCI addr %s\n", str);
		return -EINVAL;
	}

	if (bus > 255 || device > 31 || function > 7) {
		pr_err("netdevsim: Invalid values in PCI addr %s\n", str);
		return -EINVAL;
	}

	pci_addr->domain   = (u16)domain;
	pci_addr->bus      = (u8)bus;
	pci_addr->device   = (u8)device;
	pci_addr->function = (u8)function;
...
static struct nsim_bus_dev *
nsim_bus_dev_new(unsigned int id, char *pci_addr_str, int clk_logical_id,
		 unsigned int port_count, unsigned int num_queues,
		 unsigned int wpc)
{
	struct nsim_bus_dev *nsim_bus_dev;
	int err;
	...
	err = parse_pci_address(pci_addr_str, &nsim_bus_dev->pci_addr);
	if (err < 0) {
		kfree(nsim_bus_dev);
		return ERR_PTR(err);
	}

	/* Create the fake PCI device */
	err = create_fake_pci_device(nsim_bus_dev->pci_addr.domain,
		nsim_bus_dev->pci_addr.bus,
		nsim_bus_dev->pci_addr.device,
		nsim_bus_dev->pci_addr.function);
	if (err) {
		pr_err("Failed to create fake PCI device for PCI addr: %s\n", pci_addr_str);
		kfree(nsim_bus_dev);
		return ERR_PTR(err);
	}
	...
	nsim_bus_dev->logical_clk_id = clk_logical_id;
	nsim_bus_dev->wpc = !!wpc;
...
err_nsim_bus_dev_free:
	kfree(nsim_bus_dev);
	remove_fake_pci_device(nsim_bus_dev->pci_addr.domain,
		nsim_bus_dev->pci_addr.bus,
		nsim_bus_dev->pci_addr.device,
		nsim_bus_dev->pci_addr.function);
	return ERR_PTR(err);
...
static void nsim_bus_dev_del(struct nsim_bus_dev *nsim_bus_dev)
{
	/* Remove the fake PCI device */
	int err = remove_fake_pci_device(nsim_bus_dev->pci_addr.domain,
		nsim_bus_dev->pci_addr.bus,
		nsim_bus_dev->pci_addr.device,
		nsim_bus_dev->pci_addr.function);
	if (err) {
		pr_err("Failed to remove fake PCI device %04x:%02x:%02x:%01x: %d\n",
			nsim_bus_dev->pci_addr.domain,
			nsim_bus_dev->pci_addr.bus,
			nsim_bus_dev->pci_addr.device,
			nsim_bus_dev->pci_addr.function,
			err);
