...
#ifdef HAVE_POSIX_CLOCK_CONTEXT
...
#else /* !HAVE_POSIX_CLOCK_CONTEXT — pre-6.9 */
...
#endif /* HAVE_POSIX_CLOCK_CONTEXT */
	       ...
	       unsigned long arg)
{
	struct ptp_clock *ptp =
		container_of(pccontext->clk, struct ptp_clock, clock);
#else
long ptp_ioctl(struct posix_clock *pc, unsigned int cmd,
	       unsigned long arg)
{
...
#else
	tsevq = list_first_entry(&ptp->tsevqs,
				 struct timestamp_event_queue, qlist);
#endif
...
#else
__poll_t ptp_poll(struct posix_clock *pc, struct file *fp,
		  poll_table *wait)
{
	struct ptp_clock *ptp = container_of(pc, struct ptp_clock, clock);
	struct timestamp_event_queue *queue;

	queue = list_first_entry(&ptp->tsevqs,
				 struct timestamp_event_queue, qlist);
	poll_wait(fp, &ptp->tsev_wq, wait);
...
#else
ssize_t ptp_read(struct posix_clock *pc, uint rdflags,
		 char __user *buf, size_t cnt)
{
	struct ptp_clock *ptp = container_of(pc, struct ptp_clock, clock);
	struct timestamp_event_queue *queue;
