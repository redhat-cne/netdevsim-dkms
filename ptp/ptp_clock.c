				     ...
				     struct device *parent)
{
	...
	if (ptp->info->do_aux_work) {
		...
		ptp->kworker = kthread_create_worker(0, "nsim_ptp%d", ptp->index);
	...
	if (parent && parent->class && parent->class->name &&
	    strcmp(parent->class->name, "nsim_ptp") == 0)
	...
	if (info->pps) {
		...
		snprintf(pps.name, PPS_MAX_NAME_LEN, "nsim_ptp%d", index);
	...
	dev_set_name(&ptp->dev, "nsim_ptp%d", ptp->index);
	...
	snprintf(debugfsname, sizeof(debugfsname), "nsim_ptp%d", ptp->index);
...
struct ptp_clock_info *get_ptp_clock_info(struct ptp_clock *ptp)
{
	return ptp->info;
}
EXPORT_SYMBOL(get_ptp_clock_info);
...
static int __init ptp_init(void)
{
	...
	err = alloc_chrdev_region(&ptp_devt, 0, MINORMASK + 1, "nsim_ptp");
	if (err < 0) {
